...
const initFilters = () => {
  const filtered = postsStore?.filtered?.filtered;
  if (filtered) {
    filter.postType = filtered.postType || "all";
    filter.location = filtered.location || [];
    filter.postingTime = filtered.postingTime || null;
    filter.jobTime = filtered.jobTime || [];
  }
};
...
<template>
  <DrawerContent>
    ...
    <div class="flex flex-col mt-3 gap-5 px-5 overflow-scroll max-h-[70vh]">
      <!-- post type -->
      ...
      <div class="flex flex-col gap-2">
        ...
        >
          <Checkbox
            :id="location.id"
            :checked="filter.location.includes(location.id)"
            @update:checked="
              (event) => {
      ...
      <div class="flex flex-col gap-2">
        ...
        <div v-for="time in jobTime" :key="time.id" class="flex gap-2">
          <Checkbox
            :id="time.id"
            :checked="filter.jobTime.includes(time.id)"
            @update:checked="
              (event) => {
    ...
    <DrawerFooter>
      <DrawerClose class="flex gap-2">
        <Button @click="filterPosts" class="w-full">{{ $t("filter") }}</Button>
        <Button class="w-full" variant="outline">{{ $t("cancel") }}</Button>
      </DrawerClose>
    </DrawerFooter>
  </DrawerContent>
</template>
